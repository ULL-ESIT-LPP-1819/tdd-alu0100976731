...
RSpec::describe "Ordenación" do
  ...
  context "Pruebas de rendimiento" do
    it "Benchmark" do
      Benchmark.benchmark(CAPTION, 7, FORMAT, ">total:", ">avg:") do |x|
          fl = x.report("bucle list:")     { @lista_individuos.sort_for }
          fa = x.report("bucle array:")    { @menus.sort_for }
          el = x.report("each list:")    { @lista_individuos.sort_each }
          ea = x.report("each array:")   { @menus.sort_each }
          sl = x.report("sort list:")    { @lista_individuos.sort }
          sa = x.report("sort array:")   { @menus.sort_by{|x| x.collect{|y| y.valor_energetico}.reduce(:+)}}
          [fl+fa+el+ea+sl+sa, (fl+fa+el+ea+sl+sa)/6]
      end
    end
  end
end